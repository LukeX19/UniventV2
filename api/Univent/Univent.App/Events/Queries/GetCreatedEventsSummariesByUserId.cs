...
{
    public record GetCreatedEventsSummariesByUserIdQuery(Guid UserId, PaginationRequestDto Pagination) : IRequest<PaginationResponseDto<EventSummaryResponseDto>>;

    public class GetCreatedEventsSummariesByUserIdHandler : IRequestHandler<GetCreatedEventsSummariesByUserIdQuery, PaginationResponseDto<EventSummaryResponseDto>>
    {
        private readonly IUnitOfWork _unitOfWork;
        private readonly IMapper _mapper;

        public GetCreatedEventsSummariesByUserIdHandler(IUnitOfWork unitOfWork, IMapper mapper)
        {
            _unitOfWork = unitOfWork;
            _mapper = mapper;
        }

        public async Task<PaginationResponseDto<EventSummaryResponseDto>> Handle(GetCreatedEventsSummariesByUserIdQuery request, CancellationToken ct)
        {
            var paginatedEvents = await _unitOfWork.EventRepository.GetCreatedEventsSummariesByUserIdAsync(request.UserId, request.Pagination, ct);
